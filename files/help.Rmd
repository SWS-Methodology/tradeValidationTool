...
deals with comparisons of module and FAOSTAT aggregated
...
**Note: given that the process for generating the whole elements
of _Module/FAOSTAT comparison_ is very time-consuming, currently other
validation strategies for total trade are in course of development.**
...
Below the first three graphs, there is an infobox that shows
...
Finally, a snapshot of the data is shown at the end of the page.
...
to and, consequently, the variable you decided to correct by
considering the other variable as already correct. Thus, if you decide
to correct a quantity, you first should have a look at the unit value of
that transaction, fix it and then the imputed quantity will be given by the
ratio between the value and the corrected unit value; conversely, if you
want to correct a value, you fix the unit value and the imputed value is
obtained by multiplying the quantity by the corrected unit value.
...
saved in the correction table that can be visualised in the
...
If, sor some reason, you do not want this table to show only the outliers,
but *all* the dataset (i.e., both outlier and non-outlier observations) then
it is possible to select the **All data** in the dropdown menu under "Choose
an outlier detection method:".

Each row has a "link" in the leftmost column: if you click this,
the reporter/partner/flow/item combination will be shown in the
"Plots" tab.
...
* **perc.qty**:
  ...
  imported by the US).
* **corrected**: shows whether the observation has been already corrected.
...
integrated into the `complete_tf_m49` module.**
